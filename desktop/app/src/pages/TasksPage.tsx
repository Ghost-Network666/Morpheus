...
import { Plus, Trash2, CheckSquare, Square, ListTodo, ChevronDown } from "lucide-react";
...
export function TasksPage() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col">
      ...
      {/* Add task form */}
      <form onSubmit={addTask} className="flex items-center gap-2 border-b border-border px-6 py-3 bg-panel/30">
        <input
          value={newTitle}
        ...
        <div className="flex flex-col gap-1.5 max-w-2xl">
          {filtered.map((task) => (
            <TaskRow
              ...
              onPriorityChange={(p) => changePriority(task, p)}
            />
          ))}
        </div>
      </div>
...
function TaskRow({
  ...
  return (
    <div className={`group flex items-center gap-3 rounded-lg border border-border bg-panel/40 px-3 py-2.5 transition-colors hover:bg-panel/70 ${done ? "opacity-50" : ""}`}>
      <button onClick={onToggle} className="shrink-0 text-muted hover:text-accent transition-colors">
